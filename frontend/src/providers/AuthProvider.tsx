...
const AuthProvider = ({ children }: { children: React.ReactNode }) => {
	...
	const initAuthWithRetry = async (retryCount = 0, maxRetries = 3) => {
		try {
			...
			if (!token && retryCount < maxRetries && isSignedIn) {
				...
				console.log(`No token yet, retrying (${retryCount + 1}/${maxRetries})...`);
			...
			if (token) {
				...
				if (user && user.id) {
					...
					console.log(`User data not ready, retrying (${retryCount + 1}/${maxRetries})...`);
					setTimeout(() => initAuthWithRetry(retryCount + 1, maxRetries), 1000);
					return;
				}
			}
	...
	}, [getToken, userId, user, isUserLoaded, isSignedIn, checkAdminStatus, initSocket, disconnectSocket]);
